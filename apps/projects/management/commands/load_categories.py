...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        if options["reset"]:
            ...
            self.stdout.write(
                self.style.WARNING(f"Удалено {count} существующих категорий")
            )
        ...
        for cat_data in sub_categories:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'  Родительская категория {cat_data["parent_id"]} не найдена для {cat_data["name"]}'
                    )
                )
                continue
            ...
            status = "Создана" if created else "Обновлена"
            self.stdout.write(
                f"  {status}: {category.name} (ID: {category.kwork_id}) -> {parent.name}"
            )
