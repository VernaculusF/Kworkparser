...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument(
            "--category", type=int, help="ID категории для парсинга (kwork_id)"
        )
        parser.add_argument(
            "--all", action="store_true", help="Парсить все активные категории"
        )
        parser.add_argument(
            "--pages",
            type=int,
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if parse_all:
            ...
            if not categories.exists():
                self.stdout.write(
                    self.style.WARNING("Нет активных категорий для парсинга")
                )
                return

            self.stdout.write(
                self.style.SUCCESS(
                    f"Начинаем парсинг {categories.count()} категорий..."
                )
            )
            ...
            for category in categories:
                ...
                total_new += new_count
                self.stdout.write(
                    self.style.SUCCESS(f"Найдено новых заказов: {new_count}")
                )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"[OK] Найдено новых заказов: {new_count}")
                )
            ...
            self.stdout.write(self.style.ERROR("Укажите --category <ID> или --all"))
